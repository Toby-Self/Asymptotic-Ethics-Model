# Asymptotic Ethics Model — Full Experimental Record
...
1. **Wind-down (immediate, free, fully reversible)** — the moment a
...
2. **Assimilation (only after `abandonment_threshold` sustained empty
...
**13 tests**, `pytest test_asymptotic_ethics_model.py`, covering: base
...
1. The population-weighted-allocation test *reversed direction* at
...
2. The post-labor-economy Gini comparison was too noisy at reduced scale
...
3. A genuine bug in the test code itself: `assert x is True` fails even
...
`asymptotic_ethics_model.py`, only two comments mentioning it as
historical context.
...
- **Both previously-flagged dead-code items are now resolved**
  (`perform_rehabilitation` → `peer_rehabilitation_enabled`;
  `socialize_new_agent` → deleted) — removed from this list as of this
  revision.
